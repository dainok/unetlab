...
def main():
    ...
    if returned_data['data']['username'] != data['username']: sys.exit('name not validated')
    ...
    if returned_data['data'][username_1]['username'] != data['username']: sys.exit('name not validated')

    logging.info('Users: try to create a tiny user')
    data = {
    ...
    returned_data = make_request('https://{}/api/v1/users'.format(controller), method = 'POST', data = data, api_key = api_key, expected_code = 200)
    if returned_data['data']['email'] != data['email']: sys.exit('email not validated')
    if returned_data['data']['labels'] != None: sys.exit('labels not validated')
    if returned_data['data']['name'] != data['name']: sys.exit('name not validated')
    ...
    # modificare utente
    # get utente
    # Creare utente con ruolo sbagliato o modificare utente
