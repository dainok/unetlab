...
def test_ui_group_create_html_user(client, _user_set_group1, role):
    """Test HTML (UI) group creation."""
    ...
    if role == 'admin':
        ...
        assert len(Group.objects.filter(name=payload['name'])) == 1, (
        ...
        assert len(Group.objects.filter(name=payload['name'])) == 0, (
