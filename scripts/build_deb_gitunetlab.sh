...
RELEASE="$(cat ${SRC_DIR}/VERSION | cut -d- -f2)-git"
...
ar -cr /usr/src/unetlab_${VERSION}-${RELEASE}_${ARCH}.deb ${CONTROL_DIR}/debian-binary ${CONTROL_DIR}/control.tar.gz ${DATA_DIR}/data.tar.gz
rm -rf ${CONTROL_DIR} ${DATA_DIR}
echo /usr/src/unetlab_${VERSION}-${RELEASE}_${ARCH}.deb
