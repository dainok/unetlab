...
function apiGetLabNode($lab, $id , $html5, $username) {
	...
	if (isset($lab -> getNodes()[$id])) {
		...
		if ($node -> getNType() == 'qemu') {
			...
			//MOD by @cianfa72
			if ( $node -> getTemplate() == "bigip" || $node -> getTemplate() == "firepower6" || $node -> getTemplate() == "firepower" || $node -> getTemplate() == "linux" || $node -> getTemplate() == "xrv" || $node -> getTemplate() == "xrv9k")  {
...
function apiGetLabNodeTemplate($p) {
	...
	// First Mac MOD by @cianfa72
	if ($p['template'] == "bigip" || $p['template'] == "firepower6" || $p['template'] == "firepower" || $p['template'] == "linux" || $p['template'] == "xrv" || $p['template'] == "xrv9k") $output['data']['options']['firstmac'] =  Array(
	...
	if ($p['type'] == 'docker') {
		$output['data']['options']['console'] = Array(
			...
			'type' => 'list',
			'value' => $p['console'],
			'list' => Array('telnet' => 'telnet', 'ssh' => 'ssh' , 'rdp' => 'rdp' )
		);
	} 
	...
	//MOD by @cianfa72
	//QEMU options actually seems to be not used
	/**
	if ($p['type'] == 'qemu') {
