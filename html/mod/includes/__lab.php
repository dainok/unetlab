	...
	public function __construct($f, $tenant) {
		...
		if (!is_file($f)) {
			...
			foreach ($xml -> xpath('//lab/topology/nodes/node') as $node_id => $node) {
				...
				// F5 needs a first mac address MOD by @cianfa72
				if ( $n['template']  == "bigip" ||  $n['template']  == "firepower6" ||  $n['template']  == "firepower" ||  $n['template']  == "linux" || $n['template']  == "xrv" || $n['template']  == "xrv9k") {
				...
				foreach ($this -> getNodes() as $node_id => $node) {
					...
					switch ($node -> getNType()) {
						...
						case 'docker':
							...
							//MOD by @cianfa72
							$d -> addAttribute('console', $node -> getConsole());
							$d -> addAttribute('docker_options', $node -> getDocker_options());
							break;
						case 'qemu':
							...
							//MOD by @cianfa72
							if ( $node -> getTemplate() == "bigip" || $node -> getTemplate() == "firepower6" || $node -> getTemplate() == "firepower" || $node -> getTemplate() == "linux" || $node -> getTemplate() == "xrv" || $node -> getTemplate() == "xrv9k") $d -> addAttribute('firstmac', $node -> getFirstMac());
