...
class Node {
	...
	public function __construct($p, $id, $tenant, $lab_id) {
		...
		if ($p['type'] == 'qemu') {
			...
			//MOD by @cianfa72
			if ( $p['template']  == 'bigip' || $p['template']  == 'firepower6' || $p['template']  == 'firepower' || $p['template']  == 'linux' || $p['template']  == 'xrv' || $p['template']  == 'xrv9k') {
...
*/
		...
		if ( $html5 != 1 ) {
			switch ( $this -> console ) {
				...
				//MOD by @cianfa72
				case 'ssh' :
					//html5AddSession( $html5_db, $username.'_'.$this -> name , "ssh" , $this -> port, $this -> tenant
				
					return 'ssh://'.$_SERVER['SERVER_NAME'].':'.$this -> port;
					break;;
	...
	*/
	...
	public function getDocker_options() {
		$p = yaml_parse_file(BASE_DIR.'/html/'.TPL_DIR.'/'.$this -> template.'.yml');
		if ( $this -> docker_options == '' ) {
				return isset($p['docker_options'])?$p['docker_options']:'';
		} else {
				...
				switch ($tmpl) {
					...
					case 'xrv9k':
						for ($i = 0; $i < $this -> ethernet; $i++) {
							...
							//MOD by @cianfa72
							if ($i <= 2) {
								// Setting CMD flags for e1000 emulated network devices 
								$this -> flags_eth .= ' -device e1000'.',addr='.((int)($i/8)+3).'.'.($i%8).',multifunction=on,netdev=net'.$i.',mac='.substr($this -> firstmac, 0, 2).':'.sprintf('%02x', $this -> tenant).':'.sprintf('%02x', $this -> id / 512).':'.sprintf('%02x', $this -> id % 512).':00:'.sprintf('%02x', $i);
								$this -> flags_eth .= ' -netdev tap,id=net'.$i.',ifname=vunl'.$this -> tenant.'_'.$this -> id.'_'.$i.',script=no';
							} else {
								// Setting CMD flags (virtual device and map to TAP device)
								$mac_id = $this -> id + $i;
								$this -> flags_eth .= ' -device %NICDRIVER%,addr='.((int)($i/8)+3).'.'.($i%8).',multifunction=on,netdev=net'.$i.',mac='.substr($this -> firstmac, 0, 2).':'.sprintf('%02x', $this -> tenant).':'.sprintf('%02x', $this -> id / 512).':'.sprintf('%02x', $this -> id % 512).':00:'.sprintf('%02x', $i);
								$this -> flags_eth .= ' -netdev tap,id=net'.$i.',ifname=vunl'.$this -> tenant.'_'.$this -> id.'_'.$i.',script=no';
							}
						}
						break;
