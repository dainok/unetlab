...
function connectInterface($n, $p) {
	if (isBridge($n)) {
		...
		//MOD by @cianfa72
		error_log(date('M d H:i:s ').'DEBUG: '.'connectInterface() called for network '.$n.' and interface '.$p);
		if ($rc == 0) {
			return 0;
		} else {
...
/**
 * Function to prepare a node before starting it
 *
			...
			case 'docker':
				...
				if ($rc != 0) {
					...
					if ($n->getDocker_options() == '') {
						$cmd = 'docker -H=tcp://127.0.0.1:4243 create -ti --memory '.$n->getRam().'M --privileged -p '.$n->getPort().':'.$connPort.' --name='.$n->getUuid().' -h '.$n->getName().' '.$n->getImage();
						error_log(date('M d H:i:s ') . 'INFO: creating ' . $cmd);
						exec($cmd, $o, $rc);
					} else {
						// Docker options exist
						$cmd = 'docker -H=tcp://127.0.0.1:4243 create -ti -p '.$n->getPort().':'.$connPort.' '.$n -> getDocker_options().' --name='.$n -> getUuid().' -h '.$n -> getName().' '.$n -> getImage();
						error_log(date('M d H:i:s ') . 'INFO: creating ' . $cmd);
						exec($cmd, $o, $rc);
					}
					if ($rc != 0) {
...
function start($n, $id, $t, $nets, $scripttimeout) {
	...
	//MOD by @cianfa72
	error_log(date('M d H:i:s ').'DEBUG: '.'start() called for node '.$id.' with current status '.$n -> getStatus());
	global $node_cstart ;
	if ($n -> getStatus() !== 0) {
	...
	switch ($n -> getNType()) {
		...
		case 'docker':
			...
			// set cmd to docker_wrapper
			$cmd = '/opt/unetlab/wrappers/docker_wrapper -T '.$t.' -D '.$id.' -t '.$n -> getName().' -F /usr/bin/docker'.' -- attach '.$n -> getUuid().' > '.$n -> getRunningPath().'/wrapper.txt 2>&1 &';
			break;
		case 'vpcs':
	...
	if ($rcp == 0 && $n -> getNType() == 'docker') {
		...
		foreach ($n -> getEthernets() as $interface_id => $interface) {
			...
			// MOD by cianfa72
			if(!$interface -> getNetworkId() == 0) {
				//docker interface is connected
				if ($nets[$interface -> getNetworkId()]-> getNType() == 'bridge') {
					//network type is bridge
					$cmd = 'brctl addif vnet'.$t.'_'.$interface -> getNetworkId().' vnet'.$t.'_'.$id.'_'.$interface_id;
				} else {
					//network type is pnetX
					$cmd = 'brctl addif '.$nets[$interface -> getNetworkId()]-> getNType().' vnet'.$t.'_'.$id.'_'.$interface_id;
				}
			}
			
			// MOD by cianfa72
			error_log(date('M d H:i:s ').'DEBUG: '.'start() interface connected to network_id '.$interface -> getNetworkId());
			error_log(date('M d H:i:s ').'INFO: starting '.$cmd);
			exec($cmd, $o, $rc);
			...
			//MOD by @cianfa72 cmd line options (rename interfaces)
 ...
 */
function stop($n) {
	error_log(date('M d H:i:s ').'DEBUG: '.'stop() called with current status '.$n -> getStatus());
	if ($n -> getStatus() != 0) {
